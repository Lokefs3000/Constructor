...
PsInput VertexMain(VsInput input)
{
    PsInput output =
    {
        mul(cbVertex.Projection, float4(input.Position, 0.0, 1.0)),
        input.Color,
        input.UV
    };
...
static SamplerState ssSampler : defaultLinear;

[pixel]
float4 PixelMain(PsInput input) : SV_Target
{
    return input.Color * txTexture.Sample(ssSampler, input.UV);
}
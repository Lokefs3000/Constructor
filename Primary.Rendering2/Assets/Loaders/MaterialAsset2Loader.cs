...
{
    internal sealed class MaterialAsset2Loader : IAssetLoader
    {
        public IInternalAssetData FactoryCreateNull(AssetId id)
        {
            return new MaterialAsset2Data(id);
        }

        public IAssetDefinition FactoryCreateDef(IInternalAssetData assetData)
        {
            if (assetData is not MaterialAsset2Data materialData)
                throw new ArgumentException(nameof(assetData));

            return new MaterialAsset2(materialData);
        }
        ...
        {
            if (asset is not MaterialAsset2 material)
            ...
            try
            {
                ...
                ShaderAsset2 shader = shaderId is string ?
                    AssetManager.LoadAsset<ShaderAsset2>((string)shaderId, true) :
                    AssetManager.LoadAsset<ShaderAsset2>(new AssetId((uint)(long)shaderId), true);
                            ...
                            case ShPropertyType.Texture:
                                {
                                    ...
                                    else
                                    {
                                        block.SetResource(property.Name.GetDjb2HashCode(), assetId is string ?
                                            ...
                                            AssetManager.LoadAsset<TextureAsset>(new AssetId((uint)(long)assetId)));
                                    }

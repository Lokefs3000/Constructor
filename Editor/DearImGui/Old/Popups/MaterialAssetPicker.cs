...
{
    internal sealed class MaterialAssetPicker : IPopup
    {
        private Action<MaterialAsset> _callback;
        ...
        internal MaterialAssetPicker(Action<MaterialAsset> callback)
        {
            _callback = callback;
        ...
        public bool Render()
        {
            if (ImGui.BeginPopup("Material picker", ImGuiWindowFlags.NoMove))
            {
                if (_isFirstFrame)
                ...
                for (int i = 0; i < cached.Length; i++)
                {
                    ...
                    {
                        MaterialAsset material = AssetManager.LoadAsset<MaterialAsset>(data.Id, true);
            ...
            if (_isFirstFrame)
            {
                ...
                _isFirstFrame = !ImGui.IsPopupOpen("Material picker");
            }

            return _isFirstFrame || ImGui.IsPopupOpen("Material picker");
        }
        ...
        private void SelectAssetsBasedOnSearch(ReadOnlySpan<char> search)
        {
            ...
            AssetCategoryDatabase? category = Editor.GlobalSingleton.AssetDatabase.GetCategory<MaterialAsset>(false);
            if (category == null)
            {
                EdLog.Gui.Warning("{t} category is not within database", typeof(MaterialAsset));
                return;
            }

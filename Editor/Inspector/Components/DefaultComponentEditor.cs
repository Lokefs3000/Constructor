        ...
        public override void SetupInspectorFields(SceneEntity entity, Type type)
        {
            ...
            if (entityProperties != null)
            {
                ...
                if (cached.IsSerialized)
                {
                    ...
                    {
                        Type tempType = cached.Fields[i].Type;
                        if (!s_valueTypes.ContainsKey(tempType) && tempType.IsAssignableTo(typeof(IAssetDefinition)))
                            tempType = typeof(IAssetDefinition);

                        if (s_valueTypes.TryGetValue(tempType, out var constructor))

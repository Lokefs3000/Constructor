...
namespace Editor.Processors.Texture
{
    internal unsafe static class TextureUtil
    {
        internal static RawTextureData LoadRawData(string filePath)
        {
            byte[] rawData;
            {
                ProjectSubFilesystem? subFilesystem = AssetPipeline.SelectAppropriateFilesystem(AssetPipeline.GetFileNamespace(filePath));
                if (subFilesystem == null)
                {
                    throw new Exception($"Failed to get filesystem for: {filePath}");
                }

                using Stream? stream = FileUtility.TryWaitOpenNoThrow(Path.Combine(subFilesystem.AbsolutePath, filePath), FileMode.Open, FileAccess.Read, FileShare.Read, 5, 50);
                if (stream == null)
                {

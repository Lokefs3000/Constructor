...
namespace Editor.UI.Helpers
{
    internal unsafe sealed class StringAllocator : IDisposable
    {
        ...
        internal StringHandle Allocate(ReadOnlySpan<char> @string)
        {
            char* ptr = (char*)AllocateBytes(@string.Length);
            StringHandle handle = new StringHandle(ptr, @string.Length);
    ...
    public unsafe readonly struct StringHandle
    {
        ...
        public StringHandle(char* handle, int length)
        {
            _handle = handle;
            _length = length;
        }

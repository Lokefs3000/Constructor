...
namespace Editor.UI
{
    internal sealed class UIInteractionManager : IDisposable
    {
        ...
        internal UIInteractionManager(UIManager manager)
        {
            ...
            Editor.GlobalSingleton.EventManager.EventRecieved += EventCallback;
        }
        ...
        {
            if (Boundaries.Offset(dockHost.TabbedClientBounds, dockHost.ClientOffset).IsWithin(position))
                return dockHost;

            foreach (UIDockHost dockedHost in dockHost.DockedHosts)
            {
                if (new Boundaries(dockedHost.ClientOffset, dockedHost.ClientOffset + dockedHost.ClientSize).IsWithin(position))

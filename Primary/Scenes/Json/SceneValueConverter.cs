    ...
    internal static class SceneValueConverter
    {
        internal static bool Deserialize<TComp, T>(ref Utf8JsonReader reader, ref SceneEntity entity, out T? value) where TComp : IComponent
        {
            ...
            else
            {
                ...
                if (type == typeof(RawRenderMesh) || type == typeof(RenderMesh)) return RenderMeshConverter.TryDeserialize(ref reader, ref Unsafe.As<T?, RawRenderMesh?>(ref value));
                if (type.IsAssignableTo(typeof(IAssetDefinition))) return AssetGenericHelper<T>.TryDeserialize(ref reader, ref value);
            }

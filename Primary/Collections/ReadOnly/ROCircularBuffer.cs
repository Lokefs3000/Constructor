...
{
    [DebuggerTypeProxy(typeof(ROCircularBufferDisplay<>))]
    [DebuggerDisplay("Count = {Count}")]
    public readonly record struct ROCircularBuffer<T> : IEnumerable<T>, IReadOnlyCollection<T>
    {
        private readonly CircularBuffer<T> _array;

        public ROCircularBuffer(CircularBuffer<T> array)
        {
            _array = array;
        }
        ...
        public static implicit operator ROCircularBuffer<T>(CircularBuffer<T> array) => new ROCircularBuffer<T>(array);
    }
}

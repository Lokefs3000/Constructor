namespace Primary.RHI
{
    public struct RHIFormatInfo
    {
        ...
        public long CalculateSizeWithPitch(long rowPitch, long height, long depth = 1)
        {
            return IsBlockCompressed ?
                rowPitch * ((height + 3) / 4) * ((depth + 3) / 4) * BytesPerPixel :
                rowPitch * height * depth * (long)BytesPerPixel;
        }

...
{
    [UIElementPrettyName("SplitContainer")]
    public class UISplitContainer : UIFrame
    {
        ...
        private void EnsureMinimumSizes(UISplitPanel? panel)
        {
            ...
            Vector2 size = panel?.CurrentSize ?? Parent.CurrentSize;
        ...
        public override void MeasureSize(UIMeasureContext context)
        {
            ...
            if (Children.Count > 0)
            {
                Vector2 parentSize = CurrentSize;

                EnsureMinimumSizes(null);

                UISplitPanel first = Unsafe.As<UISplitPanel>(Children[0]);
                ...
                case UISplitDirection.Horizontal:
                    {
                        for (int i = 0; i < panels.Count; ++i)
                        {
                            ...
                            panel.RelativeOffset = new Vector2((int)parentOffset.X, (int)(offset + parentOffset.Y));
                            panel.CurrentSize = new Vector2((int)parentSize.X, height);

                            if (panel.OwnedSplits.Count > 0)
                            {
                                UISplitPanel first = panel.OwnedSplits[0];
                ...
                case UISplitDirection.Vertical:
                    {
                        for (int i = 0; i < panels.Count; ++i)
                        {
                            ...
                            panel.RelativeOffset = new Vector2((int)(offset + parentOffset.X), (int)parentOffset.Y);
                            panel.CurrentSize = new Vector2(width, (int)parentSize.Y);
        ...
        #region Properties
        [StyleableProperty(nameof(_splitColor), UIStateFlags.InvalidVisual)] public UIColor SplitColor { get => _splitColor; set => SetStyleProperty(value); }
        #endregion

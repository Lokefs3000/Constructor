...
namespace Editor.UI.Debugging
{
    public static class UIDebugRenderer
    {
        ...
        public static void Draw(IDebugRenderer renderer, UIDockHost host)
        {
            ...
            {
                renderer.PushMatrix(
                    Matrix4x4.CreateTranslation(new Vector3(-host.ClientSize.AsVector2() * 0.5f, 0.0f)) *
                    Matrix4x4.CreateScale(1.0f, -1.0f, 1.0f) *
                    Matrix4x4.CreateOrthographic(host.ClientSize.X, host.ClientSize.Y, -1.0f, 1.0f), false);

                if (Flags.HasFlag(activeWindow.RootElement.StateFlags, UIStateFlags.InvalidVisual))
                {
                    renderer.DrawWireBoundaries(activeWindow.InvalidVisualRegion, Color.Green);
                }
                ...
                {
                    Boundaries pixelCoords = element.PixelCoordinates;

                    renderer.DrawWireRect(pixelCoords.Minimum, pixelCoords.Maximum, pixelCoords.IsWithin(InputSystem.Pointer.MousePosition) ? Color.Red : Color.Yellow);
                    ...
                    if (Flags.HasFlag(element.StateFlags, UIStateFlags.InvalidVisual))
                    {
                        renderer.DrawSolidRect(pixelCoords.Minimum + new Vector2(4.0f), pixelCoords.Minimum + new Vector2(8.0f), Color.Green);
                    }

                    if (element is UISplitPanel splitPanel && splitPanel.SplitOwner != null)
                    {
                        DrawArrow(renderer, splitPanel.SplitOwner.PixelCoordinates.Center, splitPanel.PixelCoordinates.Center, Color.Blue);
                    }

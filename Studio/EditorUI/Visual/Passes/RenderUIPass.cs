...
namespace EditorUI.Visual.Passes
{
    ...
    internal sealed class RenderUIPass : IRenderPass
    {
        ...
        public void SetupRenderPasses(RenderPass renderPass, RenderContextContainer context)
        {
            ...
            if (!visual.BuiltPaints.IsEmpty)
            {
                RenderWindowData windowData = context.Get<RenderWindowData>()!;
                foreach (BuiltPaintData builtPaint in visual.BuiltPaints)
                {
                    if (builtPaint.Window != windowData.Window)
                        continue;
                    ...
                    using (RasterPassDescription desc = renderPass.SetupRasterPass("RenderUI", out PassData passData))
                    {
                        ...
                        {
                            Width = (uint)(Unsafe.SizeOf<UIVertex>() * builtPaint.MeshBuilder.Vertices.Count),
                        ...
                        {
                            Width = (uint)(Unsafe.SizeOf<ushort>() * builtPaint.MeshBuilder.Indices.Count),
                        ...
                        {
                            Width = (uint)builtPaint.DataBuffer.Length,
                            Stride = 1,
                            Usage = FGBufferUsage.Structured | FGBufferUsage.Raw | FGBufferUsage.PixelShader
                        });
        ...
        private static void PassFunction(RasterPassContext context, PassData passData)
        {
            ...
            // upload
            {
                cmd.Upload(passData.VertexBuffer, passData.BuiltData!.MeshBuilder.Vertices.AsSpan());
                cmd.Upload(passData.IndexBuffer, passData.BuiltData!.MeshBuilder.Indices.AsSpan());

                cmd.Upload(passData.DataBuffer, passData.BuiltData!.DataBuffer);
            ...
            // draw
            {
                foreach (PaintSegment segment in passData.BuiltData.Segments)
                {
                    int indexCount = segment.IndexRange.Length;
                    if (indexCount == 0)
                        continue;
                    ...
                    switch (segment.CmdType)
                    {
                        ...
                        case PaintCmdType.Text:
                            {
                                IFontTexture? texture = ((FontGlyphAtlas)segment.Argument!).FontTexture;
                                if (texture == null)
                                    continue;
                                ...
                                //passData.FontProperties!.SetResource("txFontAtlas", ((FontTexture)texture).RawTexture);
                                break;
                            }
                        case PaintCmdType.Image:
                            {
                                ...
                                if (segment.Argument is TextureAsset texture)
                                    passData.ImageProperties!.SetResource("txImage", texture);
                                else
                                    passData.ImageProperties!.SetResource("txImage", (RHITexture)segment.Argument!);
                                break;
                            }
                    }

                    cmd.SetScissor(0, segment.ClipRect.HasValue ? new FGRect(segment.ClipRect.Value) : null);
                    cmd.DrawIndexedInstanced(new FGDrawIndexedInstancedDesc((uint)indexCount, StartIndexLocation: (uint)segment.IndexRange.Start));
                }
            }
        ...
        private sealed class PassData : IPassData
        {
            ...
            public BuiltPaintData? BuiltData;
